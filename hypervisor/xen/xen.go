	...
	go func() {
		...
		if err == nil {
			...
			if res == 0 {
				...
				if err != nil {
					logrus.Error("fail to add vif to bridge: ", err.Error())
		...
		logrus.Errorf("nic %s insert failed", guest.Device)
...
func (xc *XenContext) RemoveNic(ctx *hypervisor.VmContext, n *hypervisor.InterfaceCreated, callback hypervisor.VmEvent, result chan<- hypervisor.VmEvent) {
	go func() {
		...
		logrus.Errorf("nic %s remove failed", n.DeviceName)
		result <- &hypervisor.DeviceFailed{
			Session: callback,
		}
	}()
...
func diskRoutine(add bool, xc *XenContext, ctx *hypervisor.VmContext,
	...
	logrus.Errorf("Disk %s (%s) insert %s failed", devName, filename, op)
	result <- &hypervisor.DeviceFailed{
		Session: callback,
	}
}
