package cache

import (
	"fmt"
	"sync"

	"github.com/Sirupsen/logrus"
	"github.com/hyperhq/runv/factory/base"
	"github.com/hyperhq/runv/hypervisor"
)

type cacheFactory struct {
	b         base.Factory
	cache     chan *hypervisor.Vm
	closed    chan<- int
	wg        sync.WaitGroup
	closeOnce sync.Once
}

func New(cacheSize int, b base.Factory) base.Factory {
	if cacheSize < 1 {
		return b
	}

	cache := make(chan *hypervisor.Vm)
	closed := make(chan int, cacheSize)
	c := cacheFactory{b: b, cache: cache, closed: closed}
	for i := 0; i < cacheSize; i++ {
		c.wg.Add(1)
		go func() {
			for {
				vm, err := b.GetBaseVm()
				if err != nil {
					logrus.Errorf("cache factory get error when allocate vm: %v", err)
					c.wg.Done()
					c.CloseFactory()
					return
				}
				logrus.Debugf("cache factory get vm from lower layer: %s", vm.Id)

				select {
				case cache <- vm:
					logrus.Debugf("cache factory sent one vm: %s", vm.Id)
				case _ = <-closed:
					logrus.Debugf("cache factory is going to close")
					vm.Kill()
					c.wg.Done()
					return
				}
			}
		}()
	}
	return &c
}

func (c *cacheFactory) Config() *hypervisor.BootConfig {
	return c.b.Config()
}

func (c *cacheFactory) GetBaseVm() (*hypervisor.Vm, error) {
	vm, ok := <-c.cache
	if ok {
		logrus.Debugf("cache factory get vm from cache: %s", vm.Id)
		return vm, nil
	}
	return nil, fmt.Errorf("cache factory is closed")
}

func (c *cacheFactory) CloseFactory() {
	c.closeOnce.Do(func() {
		logrus.Debugf("CloseFactory() close cache factory")
		for len(c.closed) < cap(c.closed) { // send sufficient closed signal
			c.closed <- 0
		}
		logrus.Debugf("CloseFactory() sent close signal")
		c.wg.Wait()
		close(c.cache)
		c.b.CloseFactory()
	})
}
